...
public class Kata10 {
    public static List<Map> execute() {
        ...
        List<Map> pelis =
        lists.stream().map(element -> ImmutableMap.of("name", element.get("name"),
                "videos", videos.stream().map(elem -> ImmutableMap.of("id", elem.get("id"), "title",
                        elem.get("title"))).collect(Collectors.toList()))).collect(Collectors.toList());
